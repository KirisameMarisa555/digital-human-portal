...
function Workspace() {
  ...
  useEffect(() => {
    ...
    let cancelled = false
    const restore = async () => {
      setBusy(true); setError(''); setNotice('正在恢复课程...')
      try {
        const info = await digitalHumanApi.getCourseInfo(user, courseId)
        if (cancelled) return
        setCourseName(info.name || '我的数字人课程')
        setSlides(toSlides(info.scenes || [], info.name || '课程'))
        setActive(0)
        if (info.state?.status === 'success') setStep('result')
        else if (info.state?.status === 'queued' || info.state?.status === 'running') setStep('generate')
        else setStep('edit')
        setNotice(info.state?.status === 'success' ? '已恢复已生成课程。' : `已恢复课程 ${courseId}。`)
        const urls: Record<number, string> = {}
        for (const scene of info.scenes || []) {
          try {
            const blob = await digitalHumanApi.downloadCoursePage(user, courseId, scene.Index)
            if (!cancelled) urls[scene.Index] = URL.createObjectURL(blob)
          } catch { /* 页面缩略图失败不阻止恢复讲稿 */ }
        }
        if (!cancelled) setPageUrls(urls)
      } catch (cause) {
        if (!cancelled) { localStorage.removeItem('dh-course-id'); setCourseId(''); setError((cause as Error).message); setNotice('') }
  ...
  const currentImage = current ? pageUrls[current.Index] : ''
  ...
  return <div className="shell"><header className="topbar"><div className="brand"><span className="brand-mark">D</span><span>数字人课程工作台</span></div><div className="top-actions"><span>{user}</span><button className="ghost" onClick={clear}>退出</button></div></header><main className="workspace"><section className="hero"><div><div className="eyebrow">CREATE COURSE</div><h1>{courseName}</h1><p>上传一份课件，编辑讲稿，生成数字人课程。</p>{courseId && <small className="muted">Course_Id：{courseId}</small>}</div><div className="hero-badge">数字人预设已就绪</div></section><div className="steps"><StepItem number="01" label="上传课件" active={step === 'upload'} done={step !== 'upload'} /><StepItem number="02" label="编辑讲稿" active={step === 'edit'} done={step === 'generate' || step === 'result'} /><StepItem number="03" label="生成课程" active={step === 'generate'} done={step === 'result'} /><StepItem number="04" label="查看结果" active={step === 'result'} done={false} /></div>{notice && <div className="notice">{notice}</div>}{error && <div className="error">{error}</div>}
    ...
    {step === 'edit' && <section className="editor-layout"><aside className="panel slide-list"><div className="panel-heading"><div><h2>课件页面</h2><p>{slides.length} 页 · Course_Id 已保存</p></div><span className="step">02</span></div>{slides.map((slide, index) => <button className={`slide-item ${index === active ? 'selected' : ''}`} key={slide.Index} onClick={() => setActive(index)}>{pageUrls[slide.Index] ? <img className="slide-thumb" src={pageUrls[slide.Index]} alt={`第 ${slide.Index} 页`} /> : <span>{String(slide.Index).padStart(2, '0')}</span>}<div><strong>{slide.title}</strong><small>{slide.Script ? '已有讲稿' : '待编辑'}</small></div></button>)}</aside><section className="panel editor-main"><div className="canvas">{currentImage ? <img className="canvas-image" src={currentImage} alt={`第 ${current?.Index || ''} 页`} /> : <div className="canvas-copy">{current?.Text || '页面预览加载中...'}</div>}<div className="canvas-label">页面 {current?.Index || '-'}</div></div><div className="script-box"><div className="panel-heading"><div><h2>本页讲稿</h2><p>修改后点击保存草稿。</p></div><button className="ghost" onClick={saveDraft} disabled={busy}>保存草稿</button></div><textarea value={current?.Script || ''} onChange={(event) => updateScript(event.target.value)} placeholder="请输入本页讲稿" /></div><div className="editor-actions"><button className="ghost" onClick={() => setStep('upload')}>重新上传</button><button className="primary" onClick={generate} disabled={busy || !courseId}>{busy ? '正在提交...' : '保存并生成课程'}</button></div></section></section>}
